...
export class AuthService {
    ...
    async login(commonDto: LoginDto, req: Request): Promise<Tokens> {
        try {
            const {
                auth_method,
                email,
            ...
            if (
                ...
                await this.verifyAuth(commonDto, req.ip as string);
            }
            ...
            if (auth_method === 'GOOGLE' || auth_method === 'APPLE') {
                ...
                if (user) {
                    ...
                    if (!ok) {
                        await this.authRateLimitService.handleAuthFailure(email ?? null, req.ip as string);
                        throw new BadRequestException('Invalid credentials');
                    }
                    await this.authRateLimitService.resetAuthLimits(email ?? null, req.ip as string);
                }
            }
    ...
    private async verifyAuth(payload: any, ip: string) {
        ...
        try {
            ...
            if (auth_method === 'EMAIL_PW') {
                ...
                if (!user) {
                    await this.authRateLimitService.handleAuthFailure(identifier, ip);
                    throw new BadRequestException('Invalid credentials');
                }
                ...
                await this.authRateLimitService.resetAuthLimits(identifier, ip);
                return true;
            }
